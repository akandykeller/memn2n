...
class MemN2N(object):
	...
	self._sentence_size_w_order = sentence_size_w_order
        ...
        self.filter_sizes = range(1, sentence_size + 1)
        self._num_filters = int(100.0 / len(self.filter_sizes))
        self._embedding_size = self._num_filters * len(self.filter_sizes)
	    ...
	    ipdb.set_trace()	    
            ...
            with tf.variable_scope('hop_2'):
                ...
                for i, filter_size in enumerate(self.filter_sizes):
                    with tf.variable_scope("C_conv_{}".format(filter_size)):
                        conv = tf.nn.conv2d(
                            tf.expand_dims(s_emb, -1),
                            self.C_conv_W[2][i],
                            strides=[1, 1, 1, 1],
                            padding="VALID",
                            name="conv")
                        # Apply nonlinearity
                        h = tf.nn.relu(tf.nn.bias_add(conv, self.C_conv_b[2][i]), name="relu")

                        # Maxpooling over the outputs
                        pooled = tf.nn.max_pool(
                            h,
                            ksize=[1, self._sentence_size_w_order - filter_size + 1, 1, 1],
                            strides=[1, 1, 1, 1],
                            padding='VALID',
                            name="pool")
