...
class MemN2N(object):
    ...
    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size,
        ...
        order_logit = self._order_inference(self._order_sents, self._order_s_lens, self._q_words) 
    ...
    def _build_inputs(self):
        ...
        self._order_s_lens = tf.placeholder(tf.int32, [None], name="order_stories_lens")
    ...
    def _order_inference(self, sentences, s_lens, q_words):
        with tf.variable_scope(self._name):
            ...
            c_init_state = c_cell.zero_state(tf.shape(sentences)[0], tf.float32)

        # Use last rnn encoder for test
        with tf.variable_scope('RNN_C_3', reuse=True):
            m_states = rnn.dynamic_rnn(c_cell, s_emb, initial_state=c_init_state,
                                       sequence_length=s_lens)

        m_C = m_states[-1]

        sentence_w_order_words = tf.concat(1, [m_C, q_emb[:, 0], q_emb[:, 1]])
    ...
    def order_batch_fit(self, sentences, s_lens, q_words, answers):
        """Runs the training algorithm over the passed batch
        ...
        Returns:
            loss: floating-point number, the loss computed for the batch
        """
        feed_dict = {self._order_sents: sentences, self._order_s_lens: s_lens, 
                     self._q_words: q_words, self._order_answers: answers}
    ...
    def order_predict(self, sentences, s_lens, q_words):
        """Predicts answers as one-hot encoding.
